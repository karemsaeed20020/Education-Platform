...
export default function WatchVideoPage() {
  ...
  const fetchVideo = async () => {
    try {
      console.log('🎬 Fetching video details for ID:', params.id);
      const response = await api.get(`/api/videos/${params.id}`);
      ...
      if (response.data.status === 'success') {
        ...
        // Increment view count
        await incrementViews(response.data.data.video._id);
      } else {
        console.error('❌ Video not found');
        toast.error('الفيديو غير موجود');
        router.push('/student/videos');
      ...
      console.error('Error fetching video:', error);
      const message = error.response?.data?.message || 'فشل في تحميل الفيديو';
      toast.error(message);
      router.push('/student/videos');
    } finally {
      setLoading(false);
  ...
  const likeVideo = async () => {
    ...
    try {
      setIsLiking(true);
      const response = await api.post(`/api/videos/${video._id}/like`);
      ...
      if (response.data.status === 'success') {
        ...
        toast.success(response.data.message || 'تم تحديث الإعجاب');
      } else {
        toast.error(response.data.message || 'فشل في تحديث الإعجاب');
      }
    } catch (error: any) {
      console.error('Error liking video:', error);
  ...
  const trackDownload = async (videoId: string) => {
    try {
      await api.post(`/api/videos/${videoId}/download`);
    } catch (error) {
      console.error('Error tracking download:', error);
    }
  };
