...
import { api } from '@/redux/slices/authSlice' // Adjust path as needed
...
export default function ContactPage() {
  ...
  const onSubmit = async (data: ContactFormData) => {
    try {
      ...
      if (res.data.success) {
        toast.success(res.data.message || "تم إرسال رسالتك بنجاح! سنقوم بالرد عليك قريباً.")
        reset()
      } else {
        throw new Error(res.data.message || "خطأ غير متوقع")
      }
    } catch (error: any) {
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  {contactInfo.map((item, index) => (
                    <motion.div
                      key={index}
                      className="flex items-start gap-4 p-4 rounded-2xl bg-gradient-to-l from-blue-50 to-purple-50 hover:shadow-md transition-all duration-300"
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                    >
                      {/* ✅ Fixed: Using gap-4 instead of space-x-4 for better spacing control */}
                ...
                {/* Quick Stats */}
                <motion.div 
                  className="mt-6 p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                ...
                >
                  <div className="text-center">
                    <div className="flex items-center justify-center gap-2 mb-2">
                      {/* ✅ Fixed: Using gap-2 for consistent spacing */}
