...
import { getProfile } from '@/redux/slices/authSlice'; // Keep getProfile for refreshing state
...
const ProfilePage = () => {
  ...
  const onSubmitProfile = async (formData: ProfileFormData) => {
    ...
    try {
      ...
      // Use api instance directly instead of Redux thunk
      ...
      const errorMessage = err.response?.data?.message || err.response?.data?.error || err.message || 'فشل في تحديث الملف الشخصي';
  ...
  const onSubmitPassword = async (formData: PasswordFormData) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || err.message || 'فشل في تغيير كلمة المرور';
      toast.error(errorMessage);
    } finally {
      setSaving(false);
    }
